...
Player.prototype.initialize = function(cb) {
  ...
  pend.wait(function(err) {
    ...
    for (var queueId in self.playlist) {
      ...
      if (!('pos' in queueItem)) {
        // Make sure restored tracks have the new pos attribute, for upgrading
        // from old dbs.
        queueItem.pos = -1;
      } else if (queueItem.pos != -1) {
        self.currentTrack = queueItem;
        self.pausedTime = queueItem.pos;
        self.trackStartDate = new Date(new Date().getTime() - self.pausedTime*1000);
      }
    }
    ...
    cacheAllOptions(cb);
    // Restore track and pos from last checkpoint before shutdown.
    if (self.currentTrack) {
      self.seek(self.currentTrack.id, self.pausedTime);
      self.pause();
    }
  });
  ...
  function initPlayer(cb) {
    ...
    grooveEncoder.formatShortName = "flac";
    grooveEncoder.codecShortName = "flac";
    // Ignored by flac? If you want to set AVCodecContext.compression_rate you
    // must pass it though to JS via the GNEncoder object created in
    // node_modules/groove/src/gn_encoder.cc::GNEncoder::Attach().
    grooveEncoder.bitRate = 256 * 1000;
    // See libavcodec/flacenc.c::flac_encode_init
    grooveEncoder.targetAudioFormat.sampleRate = 192000;
    // See libgroove/groove/groove.h::GrooveSampleFormat and libavutil/samplefmt.c::SampleFmtInfo
    grooveEncoder.targetAudioFormat.sampleFormat = 2; //"s32";
...
Player.prototype.streamMiddleware = function(req, resp, next) {
  var self = this;
  if (req.path.indexOf('/stream.') != 0) next();
  if (req.path !== '/stream.flac') {
    resp.setHeader('Connection', 'close');
    resp.setHeader('Location', '/stream.flac');
    resp.statusCode = 301;
    return resp.send()
  }
...
function playlistChanged(self) {
  ...
  // Lazy clear the oldCurrentTrack.pos, if any.
  self.tracksInOrder.forEach(function(track, index) {
    if (track.pos != -1 && !(self.currentTrack && track.index == self.currentTrack.index)) {
      track.pos = -1;
      self.persistPlaylistItem(track);
    }
  });
  if (self.currentTrack) {
    self.tracksInOrder.forEach(function(track, index) {
    ...
    // If pos == -1 then we have probably just switched to this track,
    // otherwise just update pos so we can start in the same place on
    // restarts.
    if (self.currentTrack.pos == -1)
      self.currentTrack.pos = 0;
    else
      self.currentTrack.pos = self.getCurPos();
    self.persistPlaylistItem(self.currentTrack);
  } else {
...
function checkUpdateGroovePlaylist(self) {
  ...
  if (currentGrooveItem) {
    ...
    if (self.seekRequestPos >= 0) {
      ...
      // Probably starting a new track with a cached pos.
      if (seekPos == 0 && self.currentTrack.pos != -1)
        seekPos = self.currentTrack.pos;
