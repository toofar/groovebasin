...
Player.prototype.refreshManualTimeInterval = function() {
  var wantManualTime = !this.hardwarePlayback && !this.desiredEncoderAttachState;
  if (wantManualTime && !this.manualTimeInterval) {
    this.manualTimeInterval = setInterval(this.checkNowPlaying.bind(this), 100);
  } else if (!wantManualTime && this.manualTimeInterval) {
    clearInterval(this.manualTimeInterval);
    this.manualTimeInterval = null;
  }
};
...
Player.prototype.attachEncoder = function(cb) {
  ...
  function flushEncoded() {
    ...
    for (;;) {
      ...
      if (bufferedSeconds > 5 && self.recentBuffersByteCount >= instantBufferBytes) return;
      ...
      if (buf.buffer) {
        if (buf.item) {
          ...
          while (self.recentBuffers.length > 0 &&
              self.recentBuffersByteCount - self.recentBuffers[0].buffer.length >= instantBufferBytes)
          {
            self.recentBuffersByteCount -= self.recentBuffers.shift().buffer.length;
          }
...
Player.prototype.secondsIntoFuture = function(groovePlaylistItem, pos) {
  if (!groovePlaylistItem || !pos) return 0;
  // TODO make sure when currentTrack is null, we don't keep buffering forever
  if (!this.currentTrack) return 0;

  var item = this.grooveItems[groovePlaylistItem.id];
  if (item === this.currentTrack) return pos - this.getCurPos();

  // the encode head has moved on to another track. The currentTrack hasn't yet
  // caught up, but we still need to return the correct value.

  var totalTime = 0;
  for (var i = this.currentTrack.index; i < item.index; i += 1) {
    var track = this.tracksInOrder[i];
    var dbFile = this.libraryIndex.trackTable[track.key];
    var duration = dbFile.duration;
    if (track === this.currentTrack) {
      totalTime += duration - this.getCurPos();
    } else if (track === item) {
      totalTime += pos;
    } else {
      totalTime += duration;
...
Player.prototype.setHardwarePlayback = function(value, cb) {
  ...
  function onAttachPlayer(err) {
    ...
    self.refreshManualTimeInterval();
    self.groovePlayer.on('nowplaying', self.checkNowPlaying.bind(self));
...
Player.prototype.checkNowPlaying = function() {
  var playHead = this.getPlayHead();
  var decodeHead = this.groovePlaylist.position();
  var prevCurrentTrack = this.currentTrack;
  if (playHead.manualTime) {
      if (!this.currentTrack) return;
      if (!this.isPlaying) return;
      var now = new Date();
      var dbFile = this.libraryIndex.trackTable[this.currentTrack.key];
      var nextTrackBegin = new Date(this.trackStartDate.getTime() + dbFile.duration * 1000);
      if (now > nextTrackBegin) {
        this.currentTrack = this.tracksInOrder[this.currentTrack.index + 1];
        this.trackStartDate = nextTrackBegin;
      }
  } else if (playHead.item) {
    console.log("checkNowPlaying set play head");
    var nowMs = (new Date()).getTime();
    var posMs = playHead.pos * 1000;
    this.trackStartDate = new Date(nowMs - posMs);
    this.currentTrack = this.grooveItems[playHead.item.id];
  } else if (!decodeHead.item) {
    // both play head and decode head are null. end of playlist.
    console.log("end of playlist");
    this.currentTrack = null;
  }
  if (prevCurrentTrack !== this.currentTrack) {
...
Player.prototype.getPlayHead = function() {
  if (this.hardwarePlayback) {
    ...
    var buf = this.recentBuffers[0];
    if (buf) {
      return buf;
    } else {
      console.log("return grooveEncoder.position()");
