...
Player.prototype.setUpHeadlessPlayback = function() {
  var self = this;

  if (self.checkPositionInterval) return;
  self.checkPositionInterval = setInterval(checkPosition, 50);
  console.log("set up headless playback");

  function checkPosition() {
    if (!self.trackStartDate) return;
    if (!self.isPlaying) return;
    if (!self.currentTrack) return;

    if (self.desiredEncoderAttachState && !self.pendingEncoderAttachDetach) {
      var encodeHead = self.grooveEncoder.position();
      var decodeHead = self.groovePlaylist.position();
      if (encodeHead.item) {
        var newCurrentTrack = self.grooveItems[encodeHead.item.id];
        if (newCurrentTrack !== self.currentTrack) {
          var nowMs = (new Date()).getTime();
          var posMs = encodeHead.pos * 1000;
          self.trackStartDate = new Date(nowMs - posMs);
          self.currentTrack = newCurrentTrack;
          playlistChanged(self);
          self.emit('currentTrack');
        }
      } else if (!decodeHead.item && self.currentTrack) {
...
Player.prototype.tearDownHeadlessPlayback = function() {
  console.log("tear down headless playback");
...
Player.prototype.setHardwarePlayback = function(value, cb) {
  ...
  if (self.hardwarePlayback) {
    self.tearDownHeadlessPlayback();
    self.groovePlayer = groove.createPlayer();
    self.groovePlayer.attach(self.groovePlaylist, onAttachPlayer);
  } else {
    self.setUpHeadlessPlayback();
    self.groovePlayer.removeListener('nowplaying', onNowPlaying);
